...
export default class Tokenizer {
  ...
  private state_text(): void {
    ...
    let c = ""
    for (c = this.it.currentChar(); this.it.hasNext(); c = this.it.nextChar()) {
      switch (c) {
        case "@":
          this.currentToken.data.push(this.it.getSlice())
          this.flushToken(new AtRuleToken(this.it.getLine(), this.it.getCh()))
  ...
  private state_atRuleAwaitData(): void {
    ...
    for (c = this.it.currentChar(); this.it.hasNext(); c = this.it.nextChar()) {
      switch (c) {
        ...
        case "\n":
          this.state = this.state_text
          this.flushToken(new TextToken(this.it.getLine(), this.it.getCh()))
          return
